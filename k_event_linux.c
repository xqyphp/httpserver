...
k_status_t event_dispatch(event_manager_t* manager)
{
	...
	while (K_TRUE)
	{
		log_debug("wait epfd->%d", manager->epfd);
		int nfds = epoll_wait(manager->epfd, events, 20, 500);
		for (i = 0; i < nfds; i++) {
			if ((&(manager->ev_data)) == events[i].data.ptr)
			{
				log_debug("event->new client");
				log_debug("manager->server_socket->%d", manager->server_socket);
...
k_status_t event_magager_destroy(event_manager_t* manager)
{
	close(manager->epfd);
